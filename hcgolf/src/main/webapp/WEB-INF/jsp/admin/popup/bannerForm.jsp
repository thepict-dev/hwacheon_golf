	...
	<script>
		function fn_submit(v_command){
			...
			if (v_command == "insert"){
				if (confirm('등록하시겠습니까?')) {
					...
					$("#frm").attr("action", "/_admin/popup/bannerInsert.do");
					$("#frm").submit();
				}
			}else {
				if (confirm('수정하시겠습니까?')) {
					$("#startDate").val( $("#startDate").val() + " " + $("#startTime").val() );
					$("#endDate").val( $("#endDate").val() + " " + $("#endTime").val() );
					$("#frm").attr("action", "/_admin/popup/bannerUpdate.do");
					$("#frm").submit();
				}
			}
		}
		function fn_cancel(){
			if(confirm("작성중인 내용은 저장되지 않습니다. 그래도 돌아가시겠습니까?")){
				$("#frm").attr("action", "/_admin/popup/bannerList.do");
				$("#frm").submit();
			}
		}
		function fn_list(){
		...
		function fn_delete(){
			if (confirm("삭제하시겠습니까?")){
				$("#frm").attr("action", "/_admin/popup/bannerDelete.do");
				$("#frm").submit();
			}
		}
	</script>
	<div class="container-wrap">
	    <div class="breadcrumbs">
	        <ul>
	            ...
	            <li><a href="#lnk">배너 관리</a></li>
	        </ul>
	    </div>
	    <div class="container scroll">
	        <h2 class="con-tit">배너 관리</h2>
					        ...
					        <div class="tbl-wrap new-tbl res">
					            ...
					            <table class="tbl03">
					                ...
					                <tbody>
										...
										<%
										List<CodeVO> codeList = cb.getCodeList("CODE_000000000000021");
										if(codeList.size() > 0){
										%>
							                <tr>
						                ...
						                <!--
						                <tr>
						                    <th scope="col">모바일 이미지 <em class="essential">*</em></th>
						                    <td class="add-file">
						                        <div class="add-file-inner">
						                        	<c:if test="${ searchVO.command ne 'insert' }">
