...
public class EgovImageProcessController extends HttpServlet {
     ...
     * @throws Exception
     */
    @RequestMapping("/_cmm/fms/getImage.do")
    public void getImageInf(SessionVO sessionVO, ModelMap model, @RequestParam Map<String, Object> commandMap, HttpServletResponse response) throws Exception {
		...
		String thumbnail = (String)commandMap.get("thumbnail") == null ? "" : (String)commandMap.get("thumbnail");
		String imgFlag = (String)commandMap.get("imgFlag")==null?"":(String)commandMap.get("imgFlag");

		FileVO vo = new FileVO();

		vo.setAtchFileId(atchFileId);
		vo.setFileSn(fileSn);
		vo.setImgFlag(imgFlag);
		...
		try {
			String parent = fvo.getFileStreCours();
			String child = fvo.getStreFileNm();
			if (thumbnail.equals("true")) {
				parent = EgovProperties.getProperty("Globals.thumbnailFileStorePath");
				child += "."+fvo.getFileExtsn();
			}
