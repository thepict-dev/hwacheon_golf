...
public class EgovArticleServiceImpl extends EgovAbstractServiceImpl implements EgovArticleService {
	...
	public void insertArticle(Board board) throws FdlException {
		...
		if ("Y".equals(board.getReplyAt())) {
		    ...
		    if(board.getPassword() != null && !board.getPassword().equals("")) {
		    	String enpassword = "";
				try {
					enpassword = EgovFileScrty.encryptPassword(board.getPassword(), board.getPassword());
				} catch (Exception e) {
					// TODO Auto-generated catch block
					e.printStackTrace();
				}
		    	board.setPassword(enpassword);
		    }
	...
	public void deleteAtchFileId(String atchFileId) {
		egovArticleDao.deleteAtchFileId(atchFileId);
	}

	@Override
	public BoardVO selectPrevBoardList(BoardVO board) {
		return egovArticleDao.selectPrevBoardList(board);
	}

	@Override
	public BoardVO selectNextBoardList(BoardVO board) {
		return egovArticleDao.selectNextBoardList(board);
	}

	//답변 조회
	@Override
	public BoardVO selectReplyArticle(BoardVO boardVO) {
		return egovArticleDao.selectReplyArticle(boardVO);
	}

	//답변 수정
	@Override
	public void updateReplyArticle(Board board) {
		egovArticleDao.updateReplyArticle(board);
	}

	@Override
	public int getReplyAt(BoardVO board) {
		return egovArticleDao.getReplyAt(board);
	}
	...
	public int selectPwCheck(BoardVO board) {
		...
		return egovArticleDao.selectPwCheck(board);
	}
